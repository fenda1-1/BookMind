...
- 阅读位置使用章节 ID、源文本索引、可见位置、段落索引、偏移量、可选 chunk ID、内容哈希和解析器版本，保证导入、重排和恢复后的定位稳定。
- AI 引用跳转会将 chunk 或引用文本解析为阅读位置，再回到对应原文。
- 高亮、批注、书签、目录编辑、当前阅读状态、高亮颜色和分页缓存均使用带版本的阅读记录保存。
...
发布仅由 `vX.Y.Z` 格式的 Git tag 触发。推送 tag 前，必须将 `apps/desktop/package.json`、`apps/desktop/src-tauri/tauri.conf.json` 和 `apps/desktop/src-tauri/Cargo.toml` 的版本统一更新，并新增对应的 `docs/releases/vX.Y.Z.md` 发行说明。CI 会先验证版本、完整测试和依赖审计，再并行构建 Windows、macOS、Linux 安装包；三个构建全部成功后才创建 GitHub Release 并上传安装包与 SHA-256 校验文件。
