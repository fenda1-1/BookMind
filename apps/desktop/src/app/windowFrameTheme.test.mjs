...
assert.match(appSource, /className=\{appInfoOpen \? 'app-window-info-btn active' : 'app-window-info-btn'\}/u, 'custom titlebar must expose an app information toggle');
assert.match(appSource, /BOOKMIND_REPOSITORY_URL = 'https:\/\/github\.com\/fenda1-1\/BookMind'/u, 'app information must link to the public repository');
assert.match(appSource, /BOOKMIND_ISSUES_URL/u, 'app information must provide an issue submission destination');
assert.match(appSource, /BOOKMIND_RELEASES_URL/u, 'app information must provide an update-check destination');
...
assert.match(layoutCss, /\.app-window-info-panel/, 'window frame CSS must style the app information panel');
