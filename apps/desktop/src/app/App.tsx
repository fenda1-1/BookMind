...
function AppWindowFrame({ children, className, style, title }: { children: ReactNode; className: string; style: CSSProperties; title: string }) {
  ...
  useEffect(() => {
    if (!windowSessionsOpen) return undefined;
    void refreshWindowSessions();
    const timer = window.setInterval(() => { void refreshWindowSessions(); }, 2500);
    function onPointerDown(event: PointerEvent) {
      const target = event.target instanceof Element ? event.target : null;
      if (!target?.closest('.app-window-session-menu, .app-window-session-btn')) setWindowSessionsOpen(false);
    }
    function onKeyDown(event: KeyboardEvent) {
      if (event.key === 'Escape') setWindowSessionsOpen(false);
    }
  ...
  }, [windowSessionsOpen, refreshWindowSessions]);
          ...
          {isTauriRuntime() ? (
            <div className="app-window-session" ref={windowSessionsRef}>
              <button
                ...
                onClick={() => setWindowSessionsOpen((value) => !value)}
              >
