...
const content = Array.from({ length: 2021 }, (_, index) => {
  const chapterNumber = index + 1;
  const title = chapterNumber === 1 ? '黑缎缠目' : `测试标题${chapterNumber}`;
...
const book = {
  ...
  status: '已导入',
  progress: 0,
  fileName: 'reader-model-fixture.txt',
  filePath: 'fixtures/reader-model-fixture.txt',
  coverLabel: 'TXT',
  coverTone: 'sage',
  deleted: false,
  deletedAt: '',
...
console.log(`Parsed ${chapters.length} chapters from a generated TXT fixture and verified TOC search ranges, grouping, snippets, locations, cleanup, TOC edits, hidden chapter restore/jump mapping, pagination, fixed page geometry, menu positioning, wheel paging, and highlights.`);
